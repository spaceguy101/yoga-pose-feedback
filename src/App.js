...
import { getAvgConfidence, getPoseVector, roundnum, cosineDistanceMatching } from './utils';
...
export default class PlayerControlExample extends Component {
  ...
  componentDidMount() {
    ...
    this.interval = setInterval(() => this.findSimilarity(this.state), 1000);
  ...
  findSimilarity({ pose, currentTime }) {
    ...
    if (currentTime && pose) {
      ...
      if (avgConfidence > 0.5) {
        ...
        var poseFromInputVideo = inputVideoPoses[(nearestTime / frameInterval).toString()]

        var remark = "";
        if (poseFromInputVideo) {
          var score = cosineDistanceMatching(getPoseVector(pose), poseFromInputVideo).toFixed(2)
      ...
      this.setState({ status: "Please Stand in Front of Camera with Whole body visible" })
    }
